		...
		t: (key: string, options?: { version?: string }) => {
			const translations: Record<string, string> = {
				...
				"chat:announcement.release.gpt54":
					"Poe Provider: Added Poe as an AI provider so you can access Poe models directly in Lite-Agent.",
			...
			if (key === "chat:announcement.title") {
				return `Lite-Agent ${options?.version ?? ""} Released`
			}
...
describe("Announcement", () => {
	it("renders the v3.52.0 announcement title and highlights", () => {
		...
		expect(screen.getByText("Lite-Agent 3.52.0 Released")).toBeInTheDocument()
		expect(
			screen.getByText(
				"Poe Provider: Added Poe as an AI provider so you can access Poe models directly in Lite-Agent.",
			),
