...
/**
 * Check if we should auto-open the Lite-Agent sidebar after switching to a worktree.
...
async function checkWorktreeAutoOpen(
	...
	try {
		...
		if (normalizePath(currentPath) === normalizePath(worktreeAutoOpenPath)) {
			...
			outputChannel.appendLine(`[Worktree] Auto-opening Lite-Agent sidebar for worktree: ${worktreeAutoOpenPath}`)

			// Open the Lite-Agent sidebar with a slight delay to ensure UI is ready
			setTimeout(async () => {
				try {
					await vscode.commands.executeCommand("lite-agent.plusButtonClicked")
				} catch (error) {
...
export async function activate(context: vscode.ExtensionContext) {
	...
	// Initialize the provider *before* the Lite-Agent Cloud service.
	const provider = new ClineProvider(context, outputChannel, "sidebar", contextProxy, mdmService)

	// Initialize Lite-Agent Cloud service.
